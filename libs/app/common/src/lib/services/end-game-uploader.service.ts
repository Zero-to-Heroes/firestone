...
import { BgsMetaCompositionStrategiesService, buildBgsRunStatsInput } from '@firestone/battlegrounds/services';
import { BgsGame, BgsGlobalInfoUpdateParser, GameStateFacadeService, HsGameMetaData } from '@firestone/game-state';
import {
	ArenaInfo,
	BattlegroundsInfo,
	MatchInfo,
...
export class EndGameUploaderService {
	...
	constructor(
		private readonly replayUploadService: ReplayUploadService,
	...
	public async upload2(info: UploadInfo, appVersion: { app: string; version: string }): Promise<void> {
		...
		if (!!game && !!xml) {
			...
			const bgsRunStats =
				isBattlegrounds(toGameTypeEnum(game.gameMode)) && game.bgGame
					? buildBgsRunStatsInput(game.reviewId, game, game.bgGame, user.userId!, user.username)
					: null;
			const comps = await this.bgsComps.strategies$$.getValueWithInit();
			const metadata = await this.replayUploadService.uploadGame(game, xml, bgsRunStats, comps, appVersion);
			if (!metadata) {
				console.error('[manastorm-bridge] Could not upload game, no metadata is associated to it');
				return;
			}
	...
	private async initializeGame(info: UploadInfo): Promise<{ game: GameForUpload | null; xml: string | null }> {
		...
		let playerRank: string | number | null = null;
		let seasonId: number | null = null;
		let newPlayerRank: string | number | null = null;
		...
		if (isBattlegrounds(toGameTypeEnum(game.gameMode))) {
			...
			newPlayerRank = info.battlegroundsInfoAfterGameOver?.NewRating ?? info.bgInfo?.NewRating ?? null;
			const racesFromGame =
			...
			playerRank = info.arenaInfo ? info.arenaInfo.wins + '-' + info.arenaInfo.losses : null;
			game.additionalResult = info.arenaInfo ? info.arenaInfo.wins + '-' + info.arenaInfo.losses : null;
		...
		let opponentRank;
		if (isBattlegrounds(toGameTypeEnum(game.gameMode)) || isMercenaries(game.gameMode)) {
			// Do nothing
		...
		game.xpForGame = info.xpForGame ?? null;
	...
	private async buildOpponentName(mercenariesInfo: MemoryMercenariesInfo): Promise<string | null> {
	...
	private async getMercenariesBountyDifficulty(
		mercsBountyId: number,
	): Promise<'normal' | 'heroic' | 'legendary' | null> {
		const referenceData = await this.mercenariesReferenceData.referenceData$$.getValueWithInit();
		if (!referenceData) {
			return null;
		}
