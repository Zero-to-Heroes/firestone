...
import { ARENA_DISCOVERS_DAILY_FREE_USES } from '@firestone/shared/common/service';
import {
...
@Injectable()
export class ArenaDiscoversGuardianService extends AbstractFacadeService<ArenaDiscoversGuardianService> {
	...
	constructor(protected override readonly windowManager: WindowManagerService) {
		super(windowManager, 'ArenaDiscoversGuardianService', () => !!this.freeUsesLeft$$);
	}
	...
	protected async init() {
		this.freeUsesLeft$$ = new BehaviorSubject<number>(ARENA_DISCOVERS_DAILY_FREE_USES);
		...
		console.log('[arena-discover-guardian] use count in init', today, todaysCount);
		this.freeUsesLeft$$.next(Math.max(0, ARENA_DISCOVERS_DAILY_FREE_USES - todaysCount));
