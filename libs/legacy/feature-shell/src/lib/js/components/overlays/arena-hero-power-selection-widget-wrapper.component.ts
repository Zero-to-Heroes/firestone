...
@Component({
	standalone: false,
	selector: 'arena-hero-power-selection-widget-wrapper',
	styleUrls: ['../../../css/component/overlays/background-widget.component.scss'],
	template: ` <arena-hero-power-selection class="widget" *ngIf="showWidget$ | async"></arena-hero-power-selection> `,
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ArenaHeroPowerSelectionWidgetWrapperComponent
		...
		this.showWidget$ = combineLatest([
			...
			mergeMap(([displayFromPrefs, currentScene]) => {
				...
				return this.arenaDraftManager.currentStep$$.pipe(
					this.mapData((currentStep) => currentStep === DraftSlotType.DRAFT_SLOT_HERO_POWER),
				);
			}),
