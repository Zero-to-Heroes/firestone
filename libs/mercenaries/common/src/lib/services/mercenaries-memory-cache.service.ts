...
export const MERCENARIES_SCENES: readonly (SceneMode | null)[] = [
...
export class MercenariesMemoryCacheService extends AbstractFacadeService<MercenariesMemoryCacheService> {
	public memoryCollectionInfo$$: SubscriberAwareBehaviorSubject<MemoryMercenariesCollectionInfo | null>;
	public memoryMapInfo$$: SubscriberAwareBehaviorSubject<MemoryMercenariesInfo | null>;

	private internalSubscriber$$: SubscriberAwareBehaviorSubject<null>;

	private previousScene: SceneMode | null;
	...
	private shouldFetchMercenariesMemoryInfo(newScene: SceneMode | null): boolean {
		if (!newScene || !SCENE_WITH_RELEVANT_MERC_INFO.includes(newScene)) {
			return false;
		}
	...
	private async getMercenariesMergedCollectionInfo(
		...
		const newMercenariesInfo: MemoryMercenariesCollectionInfo | null =
