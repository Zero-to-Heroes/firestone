...
import { NonFunctionProperties, updateFirstElementWithoutProp } from '@firestone/shared/framework/common';
...
export class MercenariesBattleTeam {
	...
	public getMercenary(entityId: number): BattleMercenary | undefined {
...
export class BattleMercenary {
	...
	public getAbility(entityId): BattleAbility | undefined {
		...
		const newAbilities = isEntityIdPresent
			...
			: isCardIdPresent
				...
				: hasElementWithoutEntityId
					? updateFirstElementWithoutProp(
							this.abilities ?? [],
							(ability: BattleAbility) => ability.entityId,
							base,
						)
					: [...(this.abilities ?? []), base];
