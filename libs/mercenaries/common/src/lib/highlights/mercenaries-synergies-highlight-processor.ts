...
import { normalizeMercenariesCardId } from '../services/mercenaries-utils';
import { HighlightSelector } from './mercenaries-synergies-highlight.service';

export const buildSelector = (cardId: string, allCards: CardsFacadeService): HighlightSelector | undefined => {
...
const combo = (second: ReferenceCard): HighlightSelector => {
	return (first: ReferenceCard) => (first?.cost ?? 0) <= (second?.cost ?? 0);
};
...
const speedIsOdd = (card: ReferenceCard) => isAbility(card) && (card?.cost ?? 0) % 2 === 1;
const speedIsEven = (card: ReferenceCard) => isAbility(card) && (card?.cost ?? 0) % 2 === 0;
