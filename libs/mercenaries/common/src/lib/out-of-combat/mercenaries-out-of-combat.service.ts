...
export class MercenariesOutOfCombatService {
	public store$ = new BehaviorSubject<MercenariesOutOfCombatState | null>(null);
	...
	private async processEvent(changes: MemoryUpdate, currentScene: SceneMode | null): Promise<void> {
		if (!currentScene) {
			return;
		}
