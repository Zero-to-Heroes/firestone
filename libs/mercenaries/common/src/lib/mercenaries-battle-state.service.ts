...
export class MercenariesBattleStateService {
	...
	private async processEvent(event: GameEvent | null): Promise<void> {
		if (!event) {
			return;
		}
		...
		try {
			...
			for (const parser of parsers) {
				state = await parser.parse(state, event);
			}
	...
	private async emitState(newState: MercenariesBattleState | null): Promise<void> {
		if (!newState) {
			return;
		}
	...
	private async init() {
		...
		this.internalEventSubject$
			.pipe(
				filter((event) => !!event),
				concatMap((event) => this.processEvent(event)),
			)
			.subscribe();
		this.battleState$$
			.pipe(
	...
	private getParsersFor(type: string, battleState: MercenariesBattleState | null): readonly MercenariesParser[] {
		if (!battleState) {
			return [];
		}
	...
	private buildEventEmitters() {
		const result = [(state) => this.battleState$$.next(state)];
