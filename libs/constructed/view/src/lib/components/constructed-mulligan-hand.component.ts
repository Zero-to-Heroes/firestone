	...
	async ngAfterContentInit() {
		...
		this.helpTooltip$ = combineLatest([
			...
			this.mapData(([hasPremiumSub, freeUsesLeft, statsSource]) => {
				const source = this.i18n.translateString(`decktracker.overlay.mulligan.stats-source.${statsSource}`);
				if (hasPremiumSub) {
					return this.i18n.translateString(
						'decktracker.overlay.mulligan.mulligan-impact-tooltip-premium-with-source',
						{
							source,
						},
					);
				...
				return this.i18n.translateString(
					'decktracker.overlay.mulligan.mulligan-impact-tooltip-free-with-source',
					{
						value: freeUsesLeft,
						source,
					},
			...
			this.mapData(([noData, guide]) => {
				return guide!.cardsInHand
					...
					.map((advice) => ({
						...
						personalKeptColor: buildColor(
							'hsl(112, 100%, 64%)',
							'hsl(0, 100%, 64%)',
							advice?.personalKeepRate ?? 0,
							0.6,
							0.4,
						),
						personalImpactColor: buildColor(
							'hsl(112, 100%, 64%)',
							'hsl(0, 100%, 64%)',
							advice?.personalScore ?? 0,
							4,
							-4,
						),
					}));
