...
import { ConstructedMulliganGuideService, MulliganDeckData } from '@firestone/constructed/common';
...
@Component({
	standalone: false,
	selector: 'constructed-mulligan-deck-browser',
	styleUrls: ['./constructed-mulligan-deck-browser.component.scss'],
	template: `
		<mulligan-deck-view
...
export class ConstructedMulliganDeckBrowserComponent extends AbstractSubscriptionComponent implements AfterContentInit {
	@Input() set deckstring(value: string | null | undefined) {
		this.deckstring$$.next(value ?? null);
	}
	...
	async ngAfterContentInit() {
		await waitForReady(this.prefs, this.patches, this.mulligan, this.gameStats);
	...
	cyclePlayCoin = async () => {
		const prefs = await this.prefs.getPreferences();
		const currentPlayCoin = prefs.decktrackerMulliganPlayCoinOoc ?? 'all';
		const options: readonly ('play' | 'coin' | 'all')[] = ['play', 'coin', 'all'];
		const nextPlayCoin = options[(options.indexOf(currentPlayCoin) + 1) % options.length];
		const newPrefs: Preferences = {
			...prefs,
	...
	cycleTime = async () => {
		const prefs = await this.prefs.getPreferences();
		const currentTime = prefs.decktrackerMulliganTime;
		const options: readonly ('last-patch' | 'past-3' | 'past-7')[] = ['last-patch', 'past-3', 'past-7'];
		const nextTime = options[(options.indexOf(currentTime) + 1) % options.length];
		const newPrefs: Preferences = {
			...prefs,
