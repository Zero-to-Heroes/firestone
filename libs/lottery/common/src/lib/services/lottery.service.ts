...
import { ApiRunner, CardsFacadeService, LocalStorageService, waitForReady } from '@firestone/shared/framework/core';
...
export class LotteryService {
	...
	private lotterySeason: string | null = null;
	...
	private async init() {
		this.lottery$$.onFirstSubscribe(async () => {
			await waitForReady(this.widgetController);

			const lotteryConfig: LotterySeasonConfig | null = await this.loadLotteryConfig();
			if (!lotteryConfig) {
				console.warn('[lottery] no lottery config, ignoring event');
				return;
			}
	...
	private async listenToGameEvents() {
		...
		this.eventsQueue$$.pipe(filter((event) => !!event)).subscribe((event) => {
	...
	private confirmLotteryPoints() {
		...
		if (!lotteryPoints) {
			console.warn('[lottery] no current lottery points, ignoring event');
			return;
		}
	...
	private async loadLotteryConfig(): Promise<LotterySeasonConfig | null> {
		const allSeasons = await this.api.callGetApi<readonly LotterySeasonConfig[]>(LOTTERY_SEASONS_URL);
		if (!allSeasons) {
			console.warn('[lottery] no lottery seasons, using default season');
			return null;
		}
