	...
	ngAfterContentInit() {
		this.tiers$ = combineLatest([this.stats$$, this.sort$$, this.hideLowData$$]).pipe(
			...
			this.mapData(([stats, heroSort, hideLowData]) => {
				...
				switch (heroSort) {
					case 'games-played':
						return buildMonoTier([...filteredStats].sort(sortByProperties((s) => [-s.playerRunsPlayed])));
					case 'player-winrate':
						return buildMonoTier([...filteredStats].sort(sortByProperties((s) => [-s.playerWinrate])));
					case 'global-winrate':
					default:
