type ProcessWithElectronType = NodeJS.Process & { type?: string };
...
export const isMainProcess = () => {
	const proc = process as ProcessWithElectronType;
	return (
		typeof window === 'undefined' && typeof process !== 'undefined' && (!proc.type || proc.type === 'browser')
	);
};
