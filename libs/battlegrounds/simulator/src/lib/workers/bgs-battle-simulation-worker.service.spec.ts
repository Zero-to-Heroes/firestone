...
jest.mock('@firestone/battlegrounds/core', () => ({ BgsBattleSimulationExecutorService: class {} }));
...
describe('BgsBattleSimulationWorkerService (persistent worker)', () => {
	...
	it('initializes the worker once and reuses it across sims', () => {
		...
		expect(worker.posted[1].id).not.toBe(worker.posted[2].id);

		// Final results route to the right caller by id
