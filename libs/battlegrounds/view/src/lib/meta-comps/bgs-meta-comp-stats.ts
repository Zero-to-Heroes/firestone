...
export const buildCompStats = (
	...
	const allStatsFirsts = stats
		...
		.reduce((a, b) => a + b, 0);
	const mainStats = strategies
		.map((refInfo) => {
			const s = stats.find((s) => s.archetype === refInfo.compId);
			...
			const result: BgsMetaCompStatTierItem = {
				...
				recommendedCards:
					refInfo.cards.filter((c) => c.status === 'RECOMMENDED').map((c) => buildCompCard(c, allCards)) ??
					[],
				addonCards:
