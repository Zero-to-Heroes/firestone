...
export class VolumizerBuffCounterDefinitionV2 extends CounterDefinitionV2<{ atk: number; health: number }> {
	public override id: CounterType = 'bgsVolumizerBuff';
	public override image = CardIds.ApexisGuardian_BG34_173;
	public override type: 'hearthstone' | 'battlegrounds' = 'battlegrounds';
	public override cards: readonly CardIds[] = [];

	readonly player = {
		pref: 'playerBgsVolumizerBuffCounter' as const,
		...
		value: (state: GameState, bgState: BattlegroundsState | null | undefined) => {
			...
			const value = {
				atk: getEntityTag(controllerEntity, GameTag.BACON_VOLUMIZER_ATTACK_BUFF, 0),
				health: getEntityTag(controllerEntity, GameTag.BACON_VOLUMIZER_HEALTH_BUFF, 0),
			};
		...
		setting: {
			label: (i18n: ILocalizationService): string =>
				i18n.translateString('settings.battlegrounds.overlay.counter-volumizer-buff-label'),
			tooltip: (i18n: ILocalizationService): string =>
				i18n.translateString('settings.battlegrounds.overlay.counter-volumizer-buff-tooltip'),
		},
	};
	...
	protected override tooltip(
		...
		return this.i18n.translateString(`counters.volumizer-buff.${side}`, {
			atk: atk,
			health: health,
		});
	}
