...
export const getSelectorsForArenaDraft = (cardId: string, allCards: CardsFacadeService): Selector[] => {
	const selectors: Selector[] = [];

	// Specific standard highlights for arena draft
	const selector = cardIdSelectorForArenaDraft(cardId, allCards);
	if (!!selector) {
	...
	const reverseSelector = reverseCardIdSelector(cardId, 'arena-draft', allCards);
	if (!!reverseSelector) {
