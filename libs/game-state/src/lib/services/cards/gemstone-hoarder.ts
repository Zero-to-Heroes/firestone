...
export const GemstoneHoarder: GeneratingCard & StaticGeneratingCard = {
	...
	dynamicPool: (input: StaticGeneratingCardInput) => {
		const deck = input.inputOptions.deckState;
		const oppDeck = input.inputOptions.opponentDeckState;
		const entity = deck?.findCard(input.entityId)?.card ?? oppDeck?.findCard(input.entityId)?.card;
		return entity?.relatedCardIds ?? [];
	},
