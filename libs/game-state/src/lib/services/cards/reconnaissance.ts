...
const isMatch = (c: ReferenceCard, currentClass?: string) =>
	hasCorrectType(c, CardType.MINION) && hasMechanic(c, GameTag.DEATHRATTLE) && fromAnotherClass(c, currentClass);

export const Reconnaissance: GeneratingCard & StaticGeneratingCard = {
	cardIds: [CardIds.Reconnaissance],
	publicCreator: true,
	dynamicPool: (input: StaticGeneratingCardInput) =>
		filterCards(
			Reconnaissance.cardIds[0],
			input.allCards,
