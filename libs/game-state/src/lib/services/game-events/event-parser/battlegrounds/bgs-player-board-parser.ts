...
export class BgsPlayerBoardParser implements EventParser {
	...
	async parse(currentState: GameState, gameEvent: GameEvent): Promise<GameState> {
		...
		const player: BgsPlayer | null = this.updatePlayer(currentState, playerBoard);
		const opponent: BgsPlayer | null = this.updatePlayer(currentState, opponentBoard);
		if (!player || !opponent) {
		...
		let playerTeammateBoard: PlayerBoard | null = null;
		let opponentTeammateBoard: PlayerBoard | null = null;
		...
		const newPlayers: readonly BgsPlayer[] = currentState.bgState
			.currentGame!.players.map((p) => (p.playerId === player.playerId ? player : p))
		...
		const bgsPlayer: BgsBoardInfo | null = this.buildBgsBoardInfo(playerBoard);
		const bgsOpponent: BgsBoardInfo | null = this.buildBgsBoardInfo(opponentBoard);
		const playerTeammate: BgsBoardInfo | null = this.buildBgsBoardInfo(playerTeammateBoard);
		const opponentTeammate: BgsBoardInfo | null = this.buildBgsBoardInfo(opponentTeammateBoard);
		...
		// There shouldn't be any case where the board is assigned to a face-off that is not the last, since logs
		// are procesed in order
		const lastFaceOff: BgsFaceOffWithSimulation =
			currentState.bgState.currentGame!.faceOffs[currentState.bgState.currentGame!.faceOffs.length - 1];
		if (lastFaceOff == null) {
			console.error(
	...
	private updatePlayer(currentState: GameState, playerBoard: PlayerBoard): BgsPlayer | null {
	...
	private buildBoardFromMemory(
		teammateBoardFromMemory: MemoryBgsPlayerInfo | null,
			...
			heroPowers: [
				{
					...
					entityId: teammateBoardFromMemory.HeroPower?.Tags?.find((t) => t.Name === GameTag.ENTITY_ID)
						?.Value!,
					used:
	...
	private getOpponentTeammateBoard(
		...
		const result: PlayerBoard = {
			...
			globalInfo: {
				...
				UndeadHealthBonus: opponentBoard.globalInfo?.UndeadHealthBonus ?? 0,
				...
				GoldenMinionsPlayedThisGame: opponentBoard.globalInfo?.GoldenMinionsPlayedThisGame ?? 0,
	...
	private buildBgsBoardInfo(playerBoard: PlayerBoard | null): BgsBoardInfo | null {
		if (!playerBoard) {
			return null;
		}
		...
		return {
			player: {
				...
				globalInfo: {
					...
					UndeadHealthBonus: playerBoard.globalInfo?.UndeadHealthBonus ?? 0,
					...
					GoldenMinionsPlayedThisGame: playerBoard.globalInfo?.GoldenMinionsPlayedThisGame ?? 0,
	...
	return {
		playerBoard: {
			...
			heroPowers: gameEvent.additionalData.playerBoard.heroPowers.map((heroPower) => ({
				...
				scoreValue1: heroPower.scoreValue1,
				scoreValue2: heroPower.scoreValue2,
				scoreValue3: heroPower.scoreValue3,
