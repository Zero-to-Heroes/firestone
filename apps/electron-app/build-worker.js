...
const entry = path.join(__dirname, 'src/app/services/bgs-battle-sim-worker.thread.ts');
const outfile = path.join(__dirname, '..', '..', 'dist', 'apps', 'electron-app', 'bgs-battle-sim-worker.thread.js');
...
esbuild
	...
	.then(() => {
		console.log('Bundled BGS battle sim worker to:', outfile);
	})
	.catch((err) => {
		console.error(err);
		process.exit(1);
