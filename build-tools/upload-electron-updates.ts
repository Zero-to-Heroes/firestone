...
async function getRemoteVersion(): Promise<string | null> {
	try {
		...
		const yamlContent = await result.Body.transformToString('utf-8');
		const version = parseVersionFromYaml(yamlContent);
		return version;
	} catch (error: any) {
...
async function findUpdateFiles(): Promise<Array<{ filePath: string; key: string }>> {
	...
	if (!installerFileName) {
		throw new Error(`Could not parse installer path from latest.yml. Make sure it contains a 'path:' field.`);
	}
...
async function invalidateCloudFrontCache(): Promise<void> {
	if (!CLOUDFRONT_DISTRIBUTION_ID) {
		console.log(
			'⚠️  CLOUDFRONT_DISTRIBUTION_ID environment variable not set, skipping CloudFront cache invalidation',
		);
		return;
	}
