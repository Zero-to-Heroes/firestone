...
export class AchievementNameService {
	...
	public displayName(achievementId: string) {
		return this.repo
			...
			.map(achievement => this.buildName(achievement.type, achievement.name))[0];
	}
	...
	private nameForType(type: string) {
		switch (type) {
			...
			case 'rumble_run_shrine_play':
				return 'Shrine played: ';
			case 'rumble_run_teammate_play':
				return 'Teammate joined: ';
			case 'rumble_run_passive_play':
				return 'Passive ability triggered: ';
