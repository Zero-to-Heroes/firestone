...
export class DeckbuildingEpicReq implements Requirement {
	...
	public static create(rawReq: RawRequirement, cards: AllCardsService): Requirement {
		return new DeckbuildingEpicReq(cards);
	}
	...
	private handleEvent(gameEvent: GameEvent) {
		...
		if (deck && deck.cards && deck.cards.length > 0) {
			...
			this.doesDeckMeetSpec = cards.every(card => card.rarity && card.rarity.toLowerCase() === 'epic');
		} else {
