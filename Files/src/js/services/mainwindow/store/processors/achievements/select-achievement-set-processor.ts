...
export class SelectAchievementSetProcessor implements Processor {
    ...
    public async process(event: SelectAchievementSetEvent, currentState: MainWindowState): Promise<MainWindowState> {
        const globalCategory = currentState.achievements.globalCategories
                .find((cat) => cat.achievementSets.some((set) => set.id === event.achievementSetId));
        const achievementSet: AchievementSet = globalCategory.achievementSets
                .find((set) => set.id === event.achievementSetId);
