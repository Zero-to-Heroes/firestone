...
export class AchievementStateHelper {
    ...
    public updateStateFromNewGlobalCategories(
            ...
            newGlobalCategories: ReadonlyArray<VisualAchievementCategory>): AchievementsState {
        const selectedGlobalCategory = currentState.selectedGlobalCategory != undefined 
        ...
        const selectedCategory = currentState.selectedCategory != undefined
    ...
    private updateCategories(
            existingCategories: ReadonlyArray<AchievementSet>, 
            globalCategories: ReadonlyArray<VisualAchievementCategory>): ReadonlyArray<AchievementSet> {
    ...
    private updateAchievementsList(
            existingList: ReadonlyArray<VisualAchievement>, 
            globalCategories: ReadonlyArray<VisualAchievementCategory>): ReadonlyArray<VisualAchievement> {
        const existingCategoryIds = existingList.map((achv) => achv.id);
        return globalCategories
                .map((cat) => cat.achievementSets).reduce((a, b) => a.concat(b), [])
                .map((set) => set.achievements).reduce((a, b) => a.concat(b), [])
