...
export class GalakroundPlayerCounterOverlayHandler implements OverlayHandler {
	...
	public async handleDisplayPreferences(preferences: Preferences) {
		this.showCounter = preferences.playerGalakrondCounter;
	}
	...
	public async updateOverlay(
		...
		const theWindow = await this.ow.getWindowState(OverwolfService.COUNTER_PLAYER_GALAKROND_WINDOW);
		const shouldShowWindow =
			!forceCloseWidgets &&
			(state?.playerDeck?.galakrondInvokesCount > 0 || state?.playerDeck?.containsGalakrond(this.allCards)) &&
			state.gameStarted;
		// console.log(
		// 	'should show counter?',
		...
		if (
			...
			await this.ow.obtainDeclaredWindow(OverwolfService.COUNTER_PLAYER_GALAKROND_WINDOW);
			await this.ow.restoreWindow(OverwolfService.COUNTER_PLAYER_GALAKROND_WINDOW);
		} else if (!isWindowClosed(theWindow.window_state_ex) && (!shouldShowWindow || !inGame || !this.showCounter)) {
			await this.ow.closeWindow(OverwolfService.COUNTER_PLAYER_GALAKROND_WINDOW);
		}
	}
}
