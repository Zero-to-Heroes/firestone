...
export class PogoPlayerCounterOverlayHandler implements OverlayHandler {
	...
	public async handleDisplayPreferences(preferences: Preferences) {
		this.showCounter = preferences.playerPogoCounter;
	}
	...
	public async updateOverlay(
		...
		if (
			...
			await this.ow.obtainDeclaredWindow(OverwolfService.COUNTER_PLAYER_POGO_WINDOW);
			await this.ow.restoreWindow(OverwolfService.COUNTER_PLAYER_POGO_WINDOW);
		} else if (!isWindowClosed(theWindow.window_state_ex) && (!shouldShowWindow || !inGame || !this.showCounter)) {
			await this.ow.closeWindow(OverwolfService.COUNTER_PLAYER_POGO_WINDOW);
		}
	}
}
