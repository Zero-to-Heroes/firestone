...
export class ReplaysListComponent implements AfterViewInit {
	...
	private handleProgressiveDisplay(replays: readonly GroupedReplays[]) {
		// console.debug('[replays-list] handleProgressiveDisplay');
	...
	private *buildIterator(replays: readonly GroupedReplays[]): IterableIterator<void> {
		...
		// console.debug('[replays-list] workingReplays', workingReplays);
		const step = 30;
		while (workingReplays.length > 0) {
			...
			while (
				...
				// console.debug('[replays-list] currentReplays', currentReplays);
			}
		...
		// console.debug('[replays-list] everything loaded');
		if (!(this.cdr as ViewRef)?.destroyed) {
			this.cdr.detectChanges();
		}
		return;
