...
export class BgsWinrateChartComponent {
					...
					const innerHtml = `
						<div class="body">
							<div class="section player">
								...
								<div class="value">Winrate ${currentRunDatapoint.value}</div>
							</div>
	...
	private updateInfo() {
		...
		const averageStats: readonly NumericTurnInfo[] = [];
		// console.log('computing runStats', this._stats);
		const runStats: readonly NumericTurnInfo[] = this._stats.battleResultHistory.map(
			turnInfo =>
				({
					turn: turnInfo.turn,
		...
		const lastTurn = Math.max(averageStats.length, runStats.length);
		this.lineChartLabels = [...Array(lastTurn).keys()].map(turn => '' + turn);
		// console.log('last turn is', lastTurn, this.lineChartLabels, runStats);
		this.lineChartData = [
			{
				data: averageStats.map(stat => stat.value || 0),
				label: 'Average stats for hero',
			},
			{
				data: runStats.map(stat => stat.value),
				label: 'This run',
			},
		];
		this.doResize(true);
	...
	private doResize(forceResize = false) {
		// console.log('resize in stats', this._visible, this._dirty);
		if (!forceResize && !this._visible) {
			this._dirty = true;
			return;
		}
		if (!forceResize && !this._dirty) {
			return;
		}
