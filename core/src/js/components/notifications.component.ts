...
export class NotificationsComponent implements AfterViewInit, OnDestroy {
	...
	constructor(
		...
		this.notifications$.pipe(
			...
			tap(([message, windowId]) => {
				console.log('Observable received message from bus in notification window', message.notificationId, message)
			...
			concatMap(({ message, toast }) => toast.click.pipe(tap((clickEvent: MouseEvent) => this.handleToastClick(clickEvent, message, toast.id))))
		).subscribe()
	}
	...
	private handleToastClick(event: MouseEvent, messageObject: Message, toastId: string): void {
		...
		if (
			...
			amplitude
				.getInstance()
				.logEvent('notification', { 'event': 'show-settings', 'app': messageObject.app, 'type': messageObject.type });
		...
		if (currentElement && currentElement.classList.contains('unclickable')) {
			amplitude
				.getInstance()
				.logEvent('notification', { 'event': 'unclickable', 'app': messageObject.app, 'type': messageObject.type });
	...
	private async showNotification(messageObject: Message) {
		console.log('showNotification', messageObject)
		return new Promise<void>(async resolve => {
